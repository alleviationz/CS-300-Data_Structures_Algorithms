...
public:
    ...
    int getSize() {
        // reset nodeCount before iterating
        this->nodeCount = 0;
        this->getNodeCount(root);
        return (this->nodeCount * sizeof(Node) + getSizeBSTMembers());
    }

    void Search(const string& dataId) {
        this->SearchHelper(root, dataId);
    };
...
private:
    ...
    int getSizeBSTMembers() {
        ...
        return sizeof(this) + sizeof(nodeCount) + sizeof(root);
    };

    void getNodeCount(unique_ptr<Node>& node) {
        ...
        // increase the node count (once for each node when reversing the left-side recursion, similar to InOrderPrint)
        this->nodeCount++;
    ...
    T SearchHelper(unique_ptr<Node>& node, const string& dataId) {
        ...
        if (node) {
            ...
            if (dataId == node->data.courseId) {
                cout << "Data returned: " << node->data.courseId << ": " << node->data.courseName << endl;
                return node->data;
            }
    ...
    void InsertHelper(unique_ptr<Node>& node, const T& data) {
        ...
        if (!node) {
            ...
            cout << "node set with course: " << node->data.courseId << endl;
            return;
        }
